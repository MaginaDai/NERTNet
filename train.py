...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        epoch_log = epoch + 1
        loss_train, mIoU_train, mAcc_train, allAcc_train,prototype_neg_dict = train(train_loader, model, optimizer, epoch, train_data.sub_list,prototype_neg_dict=prototype_neg_dict)
        if main_process():
        ...
        if args.evaluate and (epoch % 2 == 0 or (args.epochs<=50 and epoch%1==0)):
            loss_val, mIoU_val, mAcc_val, allAcc_val, class_miou = validate(val_loader, model, criterion, train_data.sub_list)
            if main_process():
...
def train(train_loader, model, optimizer, epoch, base_list,prototype_neg_dict=None):
    ...
    for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss, prototype_neg_dict= model(s_x=s_input, s_y=s_mask, x=input, y=target, classes = subcls, prototype_neg_dict=prototype_neg_dict)
        ...
        intersection, union, target, _ = intersectionAndUnionGPU(output, target, args.classes, args.ignore_label)
        if args.multiprocessing_distributed:
    ...
    if main_process():
        logging.info('Train result at epoch [{}/{}]: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(epoch, args.epochs, mIoU, mAcc, allAcc))
        for i in range(args.classes):
            logging.info('Class_{} Result: iou/accuracy {:.4f}/{:.4f}.'.format(i, iou_class[i], accuracy_class[i]))        
    return main_loss_meter.avg, mIoU, mAcc, allAcc, prototype_neg_dict


def validate(val_loader, model, criterion, base_list):
    if main_process():
    ...
    for e in range(10):
        for i, (input, target, s_input, s_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            intersection, union, new_target, _ = intersectionAndUnionGPU(output, target, args.classes, args.ignore_label)
